...
class GenerateText:
    ...
    def getText(self, text : str,text2: str) -> str:
        ...
        try:
            ...
            with open('l2_fenci.json', 'a', encoding='utf-8') as json_file:
...
if __name__ == '__main__':
   ...
   with open('./data/wrd_text', 'r') as file1, \
     open('./data/transcript_phn_text', 'r') as file2, \
     open('./data/phn_text', 'r') as file3:

    # 使用 zip 来同时迭代三个文件的行
    for line1, line2, line3 in zip(file1, file2, file3):
        # 这里的 line1, line2, line3 分别是三个文件的当前行
        line1 = line1.strip().split(' ',1)
        line2 = line2.strip().split(' ',1)
        line3 = line3.strip().split(' ',1)
        text = '跟读文本:'+line1[1]+'。对应的正确发音音素为：'+line2[1]+'。实际的发音音素为: '+line3[1]+'。请先对正确音素和实际音素音素进行分词，然后找到每个单词对应的音素，接着通过对比每个单词的音素，找到其中存在的替换，删除，插入错误, 然后对错误地方，给出有实际帮助的建议。'
